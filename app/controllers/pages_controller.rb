  ...
  def profile
    ...
    if @profile == current_user
      if @profile.organization
        ...
        @upcoming_organized = @profile.events.includes([photo_attachment: :blob]).select { |event| event.start_time > DateTime.now }
        @past_organized = @profile.events.includes([photo_attachment: :blob]).select { |event| event.start_time < DateTime.now }
        @new_event = Event.new
      else
        # current user's profile page for volunteer
        @upcoming_registered = @profile.registered_events.includes([user: { photo_attachment: :blob }, photo_attachment: :blob]).select { |event| event.start_time > DateTime.now }
        @past_registered = @profile.registered_events.includes([user: { photo_attachment: :blob }, photo_attachment: :blob]).select { |event| event.start_time < DateTime.now }
